...
breakServerProcess.checkRunningState().then(
     ...
     *
     * @TODO Find the difference status codes needed
     */
    function(statusCode){
        switch(statusCode){
        case 200:
...
let breakPackage = {
    ...
    /**
     * Calls the disconnected event when a window
     * is closed.
     */
    deactivate(){
    ...
    /**
     * Sets the top panels height. Emitted when
     * the atom window is resized.
     */
    _resizePanel(){
...
/**
 * Event that controls multipal changes of
 * the status bar tile.
 * @param {Object} data holds the data needed for
 * each type. Object property {type} must be set.
 */
